...
import { createModels, type Ebook } from '../../../utils/tables'
...
app.post('/', async (c) => {
  const { Ebooks } = createModels(c.env.DB)
  ...
  const created = await Ebooks.create({
    title,
    description:        (form.get('description') as string) || null,
    editorial:          (form.get('editorial') as string) || null,
    cover:              coverKey,
    pdf_file:           pdfKey,
  ...
  await logActivity(c.env.DB, 'create', 'ebook', created.id, title, coverKey)
...
app.put('/:id', async (c) => {
  ...
  await Ebooks.update(id, data)
  await logActivity(c.env.DB, 'update', 'ebook', id, title, coverKey)
...
app.delete('/:id', async (c) => {
  const { Ebooks } = createModels(c.env.DB)
  const id     = c.req.param('id')
  const entity = await Ebooks.findById(id)
  await Ebooks.delete(id)
  await logActivity(c.env.DB, 'delete', 'ebook', id, entity?.title ?? null, entity?.cover ?? null)
  return c.json({ success: true })
})
